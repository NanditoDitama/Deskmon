...
IdleChecker::IdleChecker(Deskmon *logger, QObject *parent) : QObject(parent), m_logger(logger)
{
    m_timer.setInterval(1000);
    connect(&m_timer, &QTimer::timeout, this, &IdleChecker::checkIdleTime);
    if (m_logger) {
        connect(m_logger, &Deskmon::idleThresholdChanged, this, &IdleChecker::updateIdleThresholdFromDatabase);
